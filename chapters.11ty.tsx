...
export const render = ({ chapters }: Data) => {
  ...
  const inactive: Chapter[] = [];

  chapters.sort((a, b) => a.text.localeCompare(b.text));

  for (const x of chapters)
    if (x.activity_level === "active") active.push(x);
    else inactive.push(x);

  const renderChapter = (x: Chapter) => (
    <li>
      <span>{x.icon}</span>
      {x.url != null ? (
        <a href={x.url} class="name">
          {x.text}
        </a>
      ) : (
        <span class="name">{x.text}</span>
      )}
  ...
  return (
    <>
      ...
      <section>
        ...
        <ul class="chapter-list">{active.map(renderChapter)}</ul>
      </section>

      <section>
        <h2>Inactive and former chapters</h2>
        <ul class="chapter-list">{inactive.map(renderChapter)}</ul>
      </section>
    </>
  );
};
